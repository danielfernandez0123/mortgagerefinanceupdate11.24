...
# Import scipy's lambertw, with fallback implementation if not available
try:
    from scipy.special import lambertw
except ImportError:
    # Fallback implementation of Lambert W function
    def lambertw(x):
        """
        Simple implementation of Lambert W function for x >= -1/e
        Using Newton's method
        """
        # For x close to 0, use series expansion
        if abs(x) < 1e-6:
            return x - x**2 + 1.5 * x**3

        # Initial guess
        if x > 0:
            w = np.log(max(x, 1e-16))
        else:
            w = x
...
with tab2:
    ...
    if param_choice == "Mortgage Size (M)":
        ...
        chart_data = pd.DataFrame({
            'Mortgage Size ($1000s)': M_range/1000,
            'Exact Optimal': thresholds,
            'Square Root Approx': sqrt_approx,
            'NPV Rule': npv_thresholds
        })

        st.subheader("Refinancing Threshold vs Mortgage Size (Table 1, page 20)")
        st.line_chart(chart_data.set_index('Mortgage Size ($1000s)'))
        ...
        # Create chart using Streamlit
        chart_data = pd.DataFrame({
            'Interest Rate Volatility (σ)': sigma_range,
            'Exact Optimal': thresholds,
            'Square Root Approx': sqrt_approx,
            'NPV Rule': npv_thresholds
        })

        st.subheader("Refinancing Threshold vs Interest Rate Volatility")
        st.line_chart(chart_data.set_index('Interest Rate Volatility (σ)'))
        ...
        chart_data = pd.DataFrame({
            'Marginal Tax Rate (%)': tau_range*100,
            'Refinancing Threshold (bps)': thresholds
        })

        st.subheader("Refinancing Threshold vs Tax Rate (Table 2, page 20)")
        st.bar_chart(chart_data.set_index('Marginal Tax Rate (%)'))
        ...
        chart_data = pd.DataFrame({
            'Expected Years Until Move (1/μ)': 1/mu_range,
            'Refinancing Threshold (bps)': thresholds
        })

        st.subheader("Refinancing Threshold vs Expected Time to Move (Table 3, page 21)")
        st.line_chart(chart_data.set_index('Expected Years Until Move (1/μ)'))
        ...
        # Create line chart using Streamlit
        chart_data = pd.DataFrame({
            'Real Discount Rate (%)': rho_range*100,
            'Refinancing Threshold (bps)': thresholds
        })

        st.subheader("Refinancing Threshold vs Discount Rate")
        st.line_chart(chart_data.set_index('Real Discount Rate (%)'))
        ...
        # Create line chart using Streamlit
        chart_data = pd.DataFrame({
            'Fixed Refinancing Cost ($)': cost_range,
            'Exact Optimal': thresholds,
            'Square Root Approx': sqrt_approx,
            'NPV Rule': npv_thresholds
        })

        st.subheader("Refinancing Threshold vs Fixed Costs (Related to Table 4, page 22)")
        st.line_chart(chart_data.set_index('Fixed Refinancing Cost ($)'))
...
with tab5:
    ...
    # Create visualization comparing model vs actual costs
    if 'Actual_Costs' in edited_df.columns and edited_df['Actual_Costs'].sum() > 0:
        # Use Streamlit line chart
        chart_data = pd.DataFrame({
            'Rate (%)': df_rate_to_cost['Rate'] * 100,
            'Model Predicted Costs': df_rate_to_cost['Model_Closing_Costs']
        })

        actual_data = edited_df[edited_df['Actual_Costs'] > 0]
        if not actual_data.empty:
            chart_data['Actual Lender Quotes'] = np.nan
            chart_data.loc[actual_data.index, 'Actual Lender Quotes'] = actual_data['Actual_Costs']

        st.subheader("Model Predicted vs Actual Closing Costs")
        st.line_chart(chart_data.set_index('Rate (%)'), height=400)
